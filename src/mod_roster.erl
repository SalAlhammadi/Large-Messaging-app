...
-export([webadmin_page_hostuser/4, webadmin_menu_hostuser/4, webadmin_user/5]).
...
webadmin_page_hostuser(_, Host, Username, #request{path = [<<"roster">> | RPath]} = R) ->
    ...
    Set = [make_command(add_rosteritem, R,
                        [{<<"localuser">>, Username},
                         {<<"localhost">>, Host}],
                        []),
           make_command(push_roster, R,
                        [{<<"user">>, Username},
                         {<<"host">>, Host}],
                        [])],
...
make_webadmin_roster_table(Host, Username, R, RPath) ->
    Contacts = case make_command_raw_value(get_roster, R,
                                           [{<<"user">>, Username}, {<<"host">>, Host}]) of
                   Cs when is_list(Cs) -> Cs;
                   _ -> []
               end,
    Level = 5 + length(RPath),
    Columns = [<<"jid">>, <<"nick">>, <<"subscription">>, <<"pending">>, <<"groups">>, <<"">>],
    Rows = lists:map(
             fun({Jid, Nick, Subscriptions, Pending, Groups}) ->
                     ...
                     {make_command(echo, R,
                      ...
                      ?C(Pending),
                      ?C(Groups),
                      make_command(delete_rosteritem, R,
                                   [{<<"localuser">>, Username},
                                    {<<"localhost">>, Host},
                                    {<<"user">>, element(1, JidSplit)},
                                    {<<"host">>, element(2, JidSplit)}],
                                   [{only, button}, {style, danger},
                                    {input_name_append, ejabberd_web_admin:term_to_id(
                                                          [Username,
                                                           Host,
                                                           element(1, JidSplit),
                                                           element(2, JidSplit)]
...
webadmin_user(Acc, User, Server, R, _Lang) ->
    Acc ++ [make_command(get_roster_count, R,
