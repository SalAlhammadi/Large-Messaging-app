...
process_admin(global, #request{path = [<<"vhosts">> | RPath], lang = Lang} = R, AJID) ->
    Hosts = case make_command_raw_value(registered_vhosts, R, []) of
                Hs when is_list(Hs) ->
                    Hs;
                _ ->
                    {User, Server} = R#request.us,
	            ?INFO_MSG("Access to WebAdmin page vhosts/ for account ~s@~s was denied", [User, Server]),
                    []
            end,
    ...
    VhostsElements = [make_command(registered_vhosts, R, [], [{only, presentation}]),
    ...
    make_xhtml((?H1GL((translate:translate(Lang, ?T("Virtual Hosts"))),
		      <<"basic/#xmpp-domains">>, ?T("XMPP Domains")))
               ++ VhostsElements,
	       global, Lang, AJID, Level);

process_admin(Host,  #request{path = [<<"users">>, <<"diapason">>, Diap | RPath],
			      lang = Lang} = R, AJID)
  when is_binary(Host) ->
...
process_admin(Host, #request{path = [<<"online-users">> | RPath],
			     lang = Lang} = R, AJID)
  when is_binary(Host) ->
    Level = 3 + length(RPath),
    Res = [make_command(connected_users_vhost, R, [{<<"host">>, Host}],
                        [{table_options, {2, RPath}},
                         {result_links, [{sessions, user, Level, <<"">>}]}])],
    make_xhtml([?XCT(<<"h1">>, ?T("Online Users"))] ++ Res,
	       Host, Lang, AJID, Level);
...
process_admin(Host, #request{path = [<<"node">>, SNode | NPath],
			     lang = Lang} = Request, AJID) ->
    case search_running_node(SNode) of
        false ->
            make_xhtml([?XCT(<<"h1">>, ?T("Node not found"))], Host,
                       Lang, AJID, 2);
        Node ->
            Res = get_node(Host, Node, NPath, Request#request{path = NPath}, Lang),
            Level = case Host of
...
process_admin(Host, #request{path = Path, lang = Lang} = Request, AJID) ->
    {Username, RPath} = case Path of
                   [<<"user">>, U | UPath] -> {U, UPath};
                   _ -> {unspecified, Path}
               end,
    ...
    Res = case {Host, Username} of
		  ...
		  ejabberd_hooks:run_fold(
		    webadmin_page_main, Host, [], [Request2]);
              {_, unspecified} ->
		  ejabberd_hooks:run_fold(
		    webadmin_page_host, Host, [], [Host, Request2]);
              {_Host, Username} ->
		  ejabberd_hooks:run_fold(
		    webadmin_page_hostuser, Host, [], [Host, Username, Request2])
	  end,
    Level = case Host of
    ...
    case Res of
	  ...
	  setelement(1,
		     make_xhtml([?XC(<<"h1">>, <<"Not Found">>)], Host, Lang,
				AJID, Level),
		     404);
      _ -> make_xhtml(Res, Host, Username, Lang, AJID, Level)
    end.
...
maybe_disclaimer_not_admin(MenuItems, AJID, _Lang) ->
    case {MenuItems, list_vhosts_allowed(AJID)} of
        {[_], []} ->
            [?BR,
             ?DIVRES([?C(?T("Apparently your account has no administration rights in "
...
list_users(Host, Level, PageSize, RPath, R) ->
    Usernames = case make_command_raw_value(registered_users, R, [{<<"host">>, Host}]) of
                    As when is_list(As) ->
                        As;
                    _ ->
                        {Aser, Aerver} = R#request.us,
                        ?INFO_MSG("Access to WebAdmin page users/ for account ~s@~s was denied", [Aser, Aerver]),
                        []
                end,
...
list_users(Host, Level, PageSize, RPath, R, Usernames) ->
    Columns = [<<"user">>,
               ...
               {<<"timestamp">>, left},
               {<<"status">>, left}],
    Rows = [{make_command(echo,
                          R,
                          [{<<"sentence">>, jid:encode(jid:make(Username, Host))}],
                          [{only, raw_and_value},
             ...
             make_command(get_offline_count,
                          ...
                          [{<<"user">>, Username},
                           {<<"host">>, Host}],
                          [{only, raw_and_value},
                           {result_links, [{value, arg_host, Level,
                                            <<"user/", Username/binary, "/queue/">>}]}
                          ]),
             make_command(get_roster_count,
                          ...
                          [{only, raw_and_value},
                           {result_links, [{value, arg_host, Level,
                                            <<"user/", Username/binary, "/roster/">>}]}
                          ]),
             ?C(element(1, make_command_raw_value(get_last, R, [{<<"user">>, Username}, {<<"host">>, Host}]))),
             ?C(element(2, make_command_raw_value(get_last, R, [{<<"user">>, Username}, {<<"host">>, Host}])))}
            || Username <- Usernames],
...
list_users_top(Host, Level, PageSize, RPath, R, Operation) ->
    ...
    {Command, Reverse} = case Operation of
                             <<"roster">> -> {get_roster_count, true};
                             <<"offline">> -> {get_offline_count, true};
                             <<"last">> -> {get_last, false}
                         end,
    UsernamesCounts = [{U,
                        make_command(Command,
                                     ...
                                     [{only, raw_value},
                                      {result_links, [{value, arg_host, Level,
                                                       <<"user/", U/binary, "/roster/">>}]}
                                     ])}
                       || U <- Usernames],
    USorted = lists:keysort(2, UsernamesCounts),
    UReversed = case Reverse of
                    true -> lists:reverse(USorted);
                    false -> USorted
                end,
...
get_node(global, Node, [], _Request, Lang) ->
    ...
    [?XC(<<"h1">>,
	 (str:translate_and_format(Lang, ?T("Node ~p"), [Node])))]
      ++
	[?XE(<<"ul">>, MenuItems)];
get_node(Host, Node, [], _Request, Lang) ->
...
get_node(global, Node, [<<"backup">>], R, Lang) ->
    ...
    [?XC(<<"h1">>, (str:translate_and_format(Lang, ?T("Backup of ~p"), [Node]))),
     ...
     ?XCT(<<"p">>, ?T("Import data from Prosody:")),
...
get_node(Host, Node, _NPath, Request, Lang) ->
    Res = case Host of
	      global ->
...
make_node_menu(global, Node, Lang, Level) ->
    ...
    NodeFixed = [{<<"db">>, ?T("Mnesia Tables")},
		 {<<"backup">>, ?T("Mnesia Backup")}]
...
when
    Option ::
        {only, presentation | button | result | value | raw_and_value}
...
when
    Option ::
        {only, presentation | button | result | value | raw_value | raw_and_value}
        | {input_name_append, binary()}
        | {force_execution, boolean()}
        | {table_options, {PageSize :: integer(), RemainingPath :: [binary()]}}
        | {result_named, boolean()}
...
make_command2(Name, Request, BaseArguments, Options) ->
    ...
    #request{us = {RUser, RServer}, ip = RIp, host = RHost} = Request,
    CallerInfo = #{
        usr => {RUser, RServer, <<"">>},
        ip => RIp,
        caller_host => RHost,
...
make_command2(
    ...
    ArgumentsFormatDetailed = add_arguments_details(
        ArgumentsFormat,
        C#ejabberd_commands.args_desc,
        C#ejabberd_commands.args_example
    ),
    ArgumentsEls = make_command_arguments(
    ...
    ArgumentsUsed =
        (catch lists:zip(lists:map(fun({A, _}) -> A end, ArgumentsFormat), ArgumentsUsed1)),
    ResultEls = make_command_result(
        ExecRes, ArgumentsUsed, ResultFormatApi, Automated, Resultnamed, ResultLinks, TO
    ),
...
make_command3(button, _ExecRes, _PresentationEls, [Button], _ResultEls) ->
    Button;
make_command3(result, _ExecRes, _PresentationEls, _ArgumentsEls, [{xmlcdata, _}, Xmlel]) ->
    ?XAE(<<"p">>, [{<<"class">>, <<"api">>}], [Xmlel]);
make_command3(value, _ExecRes, _PresentationEls, _ArgumentsEls, [{xmlcdata, _}, Xmlel]) ->
    Xmlel;
make_command3(value, _ExecRes, _PresentationEls, _ArgumentsEls, [{xmlel, _, _, _} = Xmlel]) ->
    Xmlel;
make_command3(raw_and_value, ExecRes, _PresentationEls, _ArgumentsEls, [{xmlel, _, _, _} = Xmlel]) ->
...
lists_zipwith3(_Combine, [], [], [], _DefaultX, _DefaultY, _DefaultZ, Res) ->
    lists:reverse(Res);
lists_zipwith3(Combine, [E1 | List1], [E2 | List2], [E3 | List3], DefX, DefY, DefZ, Res) ->
...
nice_this(This) when is_list(This) ->
    list_to_binary(
        lists:flatten(
            [string:titlecase(Word) || Word <- string:replace(This, "_", " ", all)]
        )
    ).
...
make_command_arguments(
    Name, Query, Only, Method, Style, ArgumentsFormat, BaseArguments, InputNameAppend
) ->
...
remove_base_arguments(ArgumentsFormat, BaseArguments) ->
    lists:filter(
        fun({ArgName, _ArgFormat, _ArgDesc, _ArgExample}) ->
            not lists:keymember(misc:atom_to_binary(ArgName), 1, BaseArguments)
        end,
        ArgumentsFormat
    ).

make_button_element(Name, _, Style, InputNameAppend) ->
    NameBin = <<(misc:atom_to_binary(Name))/binary, InputNameAppend/binary>>,
    NiceNameBin = nice_this(Name),
    case Style of
        danger ->
...
make_arguments_fields(Name, Query, ArgumentsFormat) ->
    lists:map(
        fun({ArgName, ArgFormat, _ArgDescription, ArgExample}) ->
            ...
            ?XE(
                ...
                [
                    ...
                    ?XE(<<"td">>, [?INPUTPH(<<"text">>, ArgLongNameBin, ArgValue, ArgExampleBin)])
                ]
            )
        end,
        ArgumentsFormat
...
execute_command(Name, Query, BaseArguments, Method, ArgumentsFormat, CallerInfo, InputNameAppend) ->
    try
        Args = prepare_arguments(Name, BaseArguments ++ Query, ArgumentsFormat),
        {Args,
            execute_command2(
                Name, Query, Args, Method, ArgumentsFormat, CallerInfo, InputNameAppend
            )}
    of
        R ->
            R
...
execute_command2(Name, Query, Arguments, Method, ArgumentsFormat, CallerInfo, InputNameAppend) ->
...
is_this_to_execute(Name, Query, Arguments, InputNameAppend) ->
    ...
    NameBin = misc:atom_to_binary(Name),
    AppendBin =
        case InputNameAppend of
            <<>> -> <<>>;
            _ -> term_to_id(Arguments)
        end,
...
make_command_result(
    ...
    ResultValueEl = make_command_result_element(
        ArgumentsUsed, Value, ResultFormatApi, ResultLinks, TO
    ),
    ResultEls =
        case Resultnamed of
            true ->
                ...
                [
                    ?XE(
                        ...
                        [
                         ...
                         || {V, {ElementName, ElementFormat}} <- lists:zip(
                                tuple_to_list(Values), TupleElements
                            )
                        ]
                    )
                ]
...
make_command_result_element(
    ...
    ContentElements = [
        {make_result(
            format_result(V, {ElementsName, ElementsFormat}),
            ElementsName,
            ArgumentsUsed,
            ResultLinks
        )}
...
make_command_result_element(ArgumentsUsed, Value, ResultFormatApi, ResultLinks, _TO) ->
    Res = make_result(
        format_result(Value, ResultFormatApi), unknown_element_name, ArgumentsUsed, ResultLinks
    ),
    Res2 =
        case Res of
            [{xmlel, _, _, _} | _] = X -> X;
...
make_result(Binary, ElementName, ArgumentsUsed, [{ResultName, arg_host, Level, Append}]) when
...
->
    ...
    ?AC(UrlBinary, Binary);
make_result(Binary, ElementName, _ArgumentsUsed, [{ResultName, host, Level, Append}]) when
    (ElementName == ResultName) or (ElementName == unknown_element_name)
->
    UrlBinary = replace_url_elements(
        [<<"server/">>, host, <<"/">>, Append],
    ...
    ?AC(UrlBinary, Binary);
make_result(Binary, ElementName, _ArgumentsUsed, [{ResultName, node, Level, Append}]) when
    (ElementName == ResultName) or (ElementName == unknown_element_name)
->
    UrlBinary = replace_url_elements(
        [<<"node/">>, node, <<"/">>, Append],
    ...
    ?AC(UrlBinary, Binary);
make_result(Binary, ElementName, _ArgumentsUsed, [{ResultName, user, Level, Append}]) when
    (ElementName == ResultName) or (ElementName == unknown_element_name)
->
    Jid =
        try jid:decode(Binary) of
            #jid{} = J -> J
        catch
            _:{bad_jid, _} ->
...
make_result(Binary, ElementName, _ArgumentsUsed, [{ResultName, shared_roster, Level, Append}]) when
    (ElementName == ResultName) or (ElementName == unknown_element_name)
->
    {GroupId, Host, _R} = jid:split(jid:decode(Binary)),
    %%+ +++ TODO This is a work in progress
    UrlBinary = replace_url_elements(
        [<<"server/">>, host, <<"/shared-roster2/">>, srg, <<"/">>, Append],
        [{host, Host}, {srg, GroupId}],
        Level
    ),
    ?AC(UrlBinary, GroupId);
make_result([{xmlcdata, _, _, _} | _] = Any, _ElementName, _ArgumentsUsed, _ResultLinks) ->
    Any;
make_result([{xmlel, _, _, _} | _] = Any, _ElementName, _ArgumentsUsed, _ResultLinks) ->
    Any;
make_result(Binary, ElementName, _ArgumentsUsed, [{ResultName, paragraph, _Level, _Append}]) when
    (ElementName == ResultName) or (ElementName == unknown_element_name)
->
    ?XC(<<"pre">>, Binary);
...
replace_url_elements(UrlComponents, Replacements, Level) ->
    ...
    Binary3 = binary:list_to_bin(lists:reverse(Binary2)),
    <<Base/binary, Binary3/binary>>.

format_result(Value, {_ResultName, integer}) when
    is_integer(Value)
->
    integer_to_binary(Value);
format_result(Value, {_ResultName, string}) when
    is_list(Value)
->
    Value;
format_result(Value, {_ResultName, string}) when
...
->
    Value;
format_result(Value, {_ResultName, atom}) when
    is_atom(Value)
->
    misc:atom_to_binary(Value);
format_result(Value, {_ResultName, any}) ->
    Value;
format_result({ok, String}, {_ResultName, restuple}) when
    is_list(String)
->
    list_to_binary(String);
format_result({error, Type, Code, Desc}, {_ResultName, restuple}) ->
    <<"Error: ", (misc:atom_to_binary(Type))/binary, " ", (integer_to_binary(Code))/binary, ": ",
...
format_result([FirstElement | Elements], {_Name, {list, ElementsDef}}) ->
    ...
    [
        ...
        | lists:map(
            fun(Element) ->
                [Separator | format_result(Element, ElementsDef)]
            end,
            Elements
        )
    ];
format_result(Value, _ResultFormat) when
    is_atom(Value)
->
    misc:atom_to_binary(Value);
format_result(Value, _ResultFormat) when
        ...
        case lists:member(<<"sort">>, RPath) of
            ...
            false ->
                GetXmlValue = fun
                    ({xmlcdata, _} = X) -> X;
                    ({xmlel, _, _, _} = X) -> X;
                    ({raw_and_value, _V, X}) -> X
                end,
                ConvertTupleToTuple = fun(Row1) ->
                    list_to_tuple(lists:map(GetXmlValue, tuple_to_list(Row1)))
                end,
...
make_table1(
    PageSize, [<<"sort">>, SortType | RPath], PageUrlBase, SortUrlBase, Start, NameOptionList, Rows1
) ->
    ColumnToSort =
        length(
            lists:takewhile(
                fun
                    (A) when A == SortType -> false;
                    ({A, _}) when A == SortType -> false;
                    (_) -> true
                end,
                NameOptionList
            )
    ...
    GetRawValue = fun
        ({xmlcdata, _} = X) -> X;
        ({xmlel, _, _, _} = X) -> X;
        ({raw_and_value, R, _X}) -> R
    end,
    GetXmlValue = fun
        ({xmlcdata, _} = X) -> X;
        ({xmlel, _, _, _} = X) -> X;
        ({raw_and_value, _R, X}) -> X
    end,
    SortTwo = fun(A1, B1) ->
...
make_table1(PageSize, [], PageUrlBase, SortUrlBase, Start, NameOptionList, Values1) ->
    ...
    [_ | SLinks1] = lists:foldl(
        fun(N, Acc) ->
            [
                ?C(<<", ">>),
                ?AC(<<SortUrlBase/binary, "sort/", N/binary, "/">>, N)
                | Acc
            ]
        end,
        [],
